...
function App() {
  ...
  async function handleDonation() {
    ...
    try {
      const preference = await createDonationPreference(selectedAmount)
  ...
  return <div className="site-shell"><Navbar /><main>
    ...
    <section className="donation section-pad" id="apoyar"><div className="donation-mark">+</div><div><p className="kicker">05 / APOYO</p><h2>Apoyá mi <em>música.</em></h2><p>Si algo de lo que hago te mueve, podés ayudarme a seguir creando, mejorando y llevando este sonido más lejos.</p></div><div className="donation-actions"><div className="amounts">{[2000, 5000, 10000, 20000].map((amount) => <button key={amount} className={selectedAmount === amount ? 'selected' : ''} onClick={() => { setSelectedAmount(amount); setDonationState('idle') }}>${amount.toLocaleString('es-AR')}</button>)}<button className="other" onClick={() => setDonationState('custom')}>OTRO MONTO</button></div><button className="button button--primary" onClick={handleDonation} disabled={donationState === 'loading'}>{donationState === 'loading' ? 'Conectando...' : 'Donar con Mercado Pago'} <ArrowUpRight size={16} /></button>{donationState === 'error' && <small className="form-error">Backend pendiente: configurá VITE_API_URL para activar Mercado Pago.</small>}{donationState === 'custom' && <small className="form-note">Podés agregar un monto personalizado desde el backend seguro.</small>}</div></section>
