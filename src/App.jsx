...
function App() {
  ...
  return <div className="site-shell"><Navbar /><main>
    ...
    <section className="booking section-pad" id="contrataciones"><div className="booking-intro"><p className="kicker">04 / TRABAJEMOS</p><h2>¿Tenés un<br /><em>proyecto?</em></h2><p>Contame qué estás imaginando. Hablemos de tu próximo lanzamiento, beat o colaboración.</p><a className="button button--ghost" href="mailto:plasonthe@gmail.com"> Gmail <ArrowUpRight size={16} /></a></div><form className="booking-form" onSubmit={handleContact}><label>Tu nombre<input name="name" required placeholder="Nombre y apellido" /></label><label>Email<input name="email" type="email" required placeholder="tu@email.com" /></label><label>WhatsApp<input name="whatsapp" placeholder="+54 9 ..." /></label><label>Servicio<select name="service" defaultValue=""><option value="" disabled>¿En qué te ayudo?</option><option>Producción musical</option><option>Beat exclusivo</option><option>Mezcla & mastering</option><option>Otro proyecto</option></select></label><label className="form-wide">Mensaje<textarea name="message" rows="4" required placeholder="Contame un poco sobre tu idea..." /></label><button className="button button--primary form-wide" type="submit" disabled={contactState === 'loading'}>{contactState === 'loading' ? 'Enviando...' : 'Enviar consulta'} <Send size={16} /></button>{contactState === 'success' && <small className="form-success" role="status">Mensaje enviado. Te respondo pronto.</small>}{contactState === 'error' && <small className="form-error" role="alert">No se pudo enviar. Revisá la configuración del servidor.</small>}</form></section>
